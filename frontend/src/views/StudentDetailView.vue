...
const timelineWithScores = computed(() =>
  timeline.value.map((e) => {
    if (e.event_type !== "exam") return e
    if (examScoreDesc.value.has(e.id)) {
      return { ...e, score_summary: examScoreDesc.value.get(e.id) }
    }
    // 还没给这个学生录入过任何成绩行（含缺考标记）→ 副标题提示
    const hasRows = (student.value?.scores ?? []).some((row) => row.exam_id === e.id)
    return hasRows ? e : { ...e, score_summary: "成绩未录入" }
  })
)
...
async function saveEdit(row) {
  ...
  try {
    await api.patch(`/results/${row.result_id}`, { score: value })
    editingId.value = null
    await load()
...
function goAddScores() {
  router.push(`/students/${props.id}/scores/new`)
}
      ...
      >
        ...
        <template #actions>
          ...
          <button class="btn" @click="addEvent">
            <Icon name="map-pin" :size="15" /> {{ t("detail.recordEvent") }}
          </button>
          <button class="btn" @click="goSummaries">
            <Icon name="clipboard" :size="15" /> {{ t("summary.open") }}
          </button>
        </template>
      </PageHeader>
